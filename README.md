...
- На Windows Server compute‑тест временно включает системный план **High Performance**,
  переводит процесс и рабочие потоки в HighQoS/High priority, калибрует доступные
  ядра и закрепляет ST за самым быстрым. Исходный план питания восстанавливается
  сразу после compute‑теста.
...
- `SFBENCH_NO_SERVER_TUNING=1` отключает автоматический performance‑режим Windows Server
  (используйте только для диагностики; такой результат не следует публиковать).
