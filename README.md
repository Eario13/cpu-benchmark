...
- Исправлена генерация scalar FP64 hot loop в GCC/Clang: константы FMA теперь
  удерживаются в регистрах, а цикл явно развёрнут в восемь раз.
- Математическая нагрузка и формула score не изменились. На тестовом двухсокетном
  Xeon Ubuntu Multi вырос с 19844,67 до 22322,67, сократив разрыв с Windows 10
  с 11,25% до 0,17%.
- MSVC и раньше создавал эквивалентный регистровый развёрнутый цикл, поэтому
  вычислительный hot loop Windows остался прежним.

[Таблица кроссплатформенных замеров и выводы](remote-tests/results/summary.md)
