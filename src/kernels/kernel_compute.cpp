...
size_t scalar_fp64_baseline(double* result, size_t iterations) {
    ...
    __m128d mul = _mm_set_sd(1.0000001);
    __m128d add = _mm_set_sd(0.0000001);

    // GCC may otherwise fold these constants into memory operands and reload
    // them for every FMA. That is mostly hidden in ST by turbo, but it puts
    // avoidable pressure on the shared load ports when both SMT siblings are
    // active. Keep the constants in vector registers on GNU-style compilers,
    // matching the register-only hot loop generated by MSVC.
#if (defined(__GNUC__) || defined(__clang__)) && !defined(_MSC_VER)
    __asm__ __volatile__("" : "+x"(mul), "+x"(add));
#endif

#define SFBENCH_SCALAR_FP64_STEP()            \
    do {                                      \
    ...
    size_t i = 0;
    NOVEC_LOOP
    for (; iterations - i >= 8; i += 8) {
        // Explicitly use the same eight-way loop unroll on every compiler.
        // The FLOP count and mathematical workload are unchanged.
        SFBENCH_SCALAR_FP64_STEP();
        SFBENCH_SCALAR_FP64_STEP();
        SFBENCH_SCALAR_FP64_STEP();
        SFBENCH_SCALAR_FP64_STEP();
        SFBENCH_SCALAR_FP64_STEP();
...
#undef SFBENCH_SCALAR_FP64_STEP
